...
void PrimitiveDrawer::DrawLine(const Vector2& p1, const Vector2& p2, const Vector4& color) {
    auto& batch = batches_[TopologyType::kLine];
    if (batch.vertices.size() + 2 > kMaxVertices) return;

    batch.vertices.push_back({ {p1.x, p1.y, 1.0f, 1.0f}, color });
    batch.vertices.push_back({ {p2.x, p2.y, 1.0f, 1.0f}, color });

}

void PrimitiveDrawer::DrawTriangle(const Vector2& p1, const Vector2& p2, const Vector2& p3, const Vector4& color, FillMode fillMode, BlendMode blendMode )
{
    ...
    batch.vertices.push_back({ {p1.x, p1.y, 1.0f, 1.0f}, color });
    batch.vertices.push_back({ {p2.x, p2.y, 1.0f, 1.0f}, color });
    batch.vertices.push_back({ {p3.x, p3.y,1.0f, 1.0f}, color });
