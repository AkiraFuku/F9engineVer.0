...
### ④ エネミー配置 (`ENEMY`)
エディター上では、単なる十字軸ではなく**ゲームと同じ敵モデル（`taru.obj` / 樽）が3Dメッシュとして自動描画**され、敵の種別ごとにテクスチャも色分けプレビューされます！

1. **ワンクリック配置（おすすめ）**:
   - 上部メニュー **[MyMenu]** またはプロパティの **[FileName & Stage Blocks]** パネルにある **[👾 エネミー手動配置]** から：
     - 🟢 **通常敵 (Normal)**: 標準の樽エネミー（`taru.png`）
     - 🔵 **バウンド敵 (Bound)**: 跳ねる樽エネミー（`taru2.png`）
     - 🔴 **追跡敵 (Chase)**: プレイヤー接近で反転追跡・突進する樽エネミー（`taru3.png`）
   - クリックするだけで、現在アクティブなレールへ自動マグネット吸着（`rail_pos`）され、即座にモデル・テクスチャ付きで配置されます。

2. **手動設定・タイプ変更**:
   - 既存のオブジェクトを選択し、**[Object Type Settings]** パネルで **[ENEMYにする]** をクリック。
   - **[Enemy Settings & Spawn]** パネルから敵タイプ（`Normal` / `Bound` / `Chase`）をいつでも1クリックで切り替えられます。
   - `Chase`（追跡敵）を選択した場合：
     - 自動的にテクスチャが `taru3.png` に切り替わります。
     - **索敵半径 (`search_radius`)**: プレイヤーを検知して追跡を開始する直線距離（デフォルト: 10.0m）
     - **見失い距離 (`lost_distance`)**: プレイヤーが離れて追跡を解除する距離（デフォルト: 14.0m）
     - **追跡突進速度 (`chase_speed`)**: 追跡時の移動速度（デフォルト: 5.5m/s）
     - **巡回移動速度 (`patrol_speed`)**: 索敵・通常時の移動速度（デフォルト: 2.0m/s）
     - これらのパラメータはスライダー等で自由に変更でき、JSONエクスポート経由でゲーム内の `ChaseEnemy` へ自動反映されます！

3. **出現方式 (スポーン設定)**:
   - `spawn_mode`:
     - `TRIGGER_SPAWN` (デフォルト): プレイヤーが手前一定距離（`spawn_distance`、デフォルト 25.0m）に近づいた瞬間にスポーンし、一度出現したら機能停止。
     - `IMMEDIATE`: ステージロード時に最初からその場に実体化。
...
| `enemy_type` | String | `"Normal"`, `"Bound"`, `"Chase"` | 敵の種類（ファクトリーに対応） |
| `search_radius` | Float | `10.0` | 追跡敵（Chase）の索敵検知半径 (m) |
| `chase_speed` | Float | `5.5` | 追跡敵（Chase）の追跡突進移動速度 (m/s) |
